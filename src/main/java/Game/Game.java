...
public class Game extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        playerName = MainMenu.playerNameTF.getText();
    ...
    private void play() {
        pacman_keyFrame = new KeyFrame(Duration.millis(110), e ->
        {
            ...
            if (bonusEaten)
                ...
                if (redGhost.caughtPacman(pacman, speed) || pinkGhost.caughtPacman(pacman, speed) ||
                        orangeGhost.caughtPacman(pacman, speed) || cyanGhost.caughtPacman(pacman, speed)) {
                    try {
                        endGame();
                    } catch (Exception ex) {
            ...
            if (pelletList.isEmpty() == true && bonusList.isEmpty()) {
                try {
                    endGame();
                } catch (Exception ex) {
                    throw new RuntimeException(ex);
    ...
    private void writeScore() {
        try {
            ...
            for (int i = 0; i < data.length - 1; i++) {
                scores[i] = Integer.parseInt(data[i].substring(0, data[i].indexOf(" ")));
            }
            int index = 5;
            ...
            System.out.println("Cant update score");
        }
    }
    ...
    private String getHighScore() {
        ...
        try {
            ...
            if (file.exists()) {
                ...
                // create the high_score.txt file and insert a 0
                setHighScore("0");
            }
        } catch (Exception e) {
    ...
    private Boolean setHighScore(String newScore) {
        ...
        try {
            File score = new File("high_score.txt");
            score.delete();
    ...
    private void drawPellets() {
        ...
        for (x = 30; x < mapWidth; x += wallSize) {
            for (y = 30; y < mapHeight; y += wallSize) {
                if (isWall(x - (wallSize / 2), y - (wallSize / 2)) == false && !isBonusFood(x, y) && (x < 50 || x > 190 || y < 90 || y > 200)) {
    ...
    private Boolean isBonusFood(double x, double y) {
        for (int n = 0; n < bonusList.size(); n++)
